...
// v98: el cache local seguro puede tener metadatos pero no el enunciado.
// Para que "Buscar pregunta" vuelva a mostrar el inicio del caso, hidratamos
// solo los resultados visibles mediante la sesión segura y mostramos únicamente
// un preview del enunciado. No se expone respuesta correcta.
...
async function resiarFetchQuestionSearchPreviewsRpc(questionIds) {
  ...
  const ids = uniqueQuestionIds(questionIds).slice(0, RESIAR_SEARCH_PREVIEW_LIMIT);
...
function resiarSearchPreviewPool(query) {
  ...
  if (!Array.isArray(pool) || !pool.length) pool = Array.isArray(preguntas) ? preguntas : [];
...
async function resiarHydrateVisibleSearchPreviews() {
  ...
  try {
    ...
    if (!resiarSecureExamEnabled()) return;

    const secure = await resiarCreateSecureExamFromCatalog(matches, {
      mode: 'search_preview',
...
function getOptimizedNavRanges(total, current) {
