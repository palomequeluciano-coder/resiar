-- ============================================================
-- RESIAR v73 - metadata de preguntas repetidas
-- Objetivo:
--   - Agregar un campo JSON para indicar en qué otros exámenes apareció una pregunta.
--   - Exponer esa metadata en el catálogo seguro y en la sesión segura.
--   - Mantener ocultas respuesta correcta/enunciado fuera de la sesión segura.
--
-- Formato recomendado de repetida_en:
-- [
--   {"examen":"CABA","anio":2006,"num_original":15,"id":"CABA_2006_15"},
--   {"examen":"UNLaM","anio":2025,"num_original":42,"id":"UNLAM_2025_42"}
-- ]
-- ============================================================

alter table public.preguntas
  add column if not exists repetida_en jsonb not null default '[]'::jsonb;

create or replace function public.get_question_catalog_v72(
  p_question_ids text[] default '{}'::text[],
  p_examen text default null,
...
declare
  ...
  v_has_ids boolean := coalesce(array_length(p_question_ids, 1), 0) > 0;
  v_examen text := nullif(btrim(coalesce(p_examen, '')), '');
...
begin
  ...
  visible as (
    select
      ...
      q.imagen_caption,
      q.repetida_en,
      coalesce(r.ord, 2147483647) as requested_ord
    from public.preguntas q
    left join requested r on r.id = q.id
    where (v_is_admin or public.pregunta_visible_para_usuario(q.examen, q.id))
      and (not v_has_ids or r.id is not null)
      and (v_examen is null or q.examen = v_examen or q.tipo = v_examen)
      and (p_anio is null or q.anio = p_anio)
    order by
      case when v_has_ids then coalesce(r.ord, 2147483647) else 2147483647 end,
      q.examen asc nulls last,
      q.anio asc nulls last,
      q.num_original asc nulls last,
      q.id asc
    limit v_limit
  )
  select
    coalesce(
      jsonb_agg(
        jsonb_build_object(
          ...
          'repetida_en', coalesce(repetida_en, '[]'::jsonb)
        )
        order by requested_ord, examen asc nulls last, anio asc nulls last, num_original asc nulls last, id asc
      ),
      '[]'::jsonb
    ),
    count(*)
...
revoke execute on function public.get_question_catalog_v72(text[], text, integer, integer) from public;
revoke execute on function public.get_question_catalog_v72(text[], text, integer, integer) from anon;
grant execute on function public.get_question_catalog_v72(text[], text, integer, integer) to authenticated;
...
revoke execute on function public.get_question_search_previews_v1(text[], integer) from public;
revoke execute on function public.get_question_search_previews_v1(text[], integer) from anon;
grant execute on function public.get_question_search_previews_v1(text[], integer) to authenticated;

-- Cierre real: la app cliente debe entrar por RPCs sanitizados, no por tabla.
revoke all privileges on table public.preguntas from anon;
revoke all privileges on table public.preguntas from authenticated;
