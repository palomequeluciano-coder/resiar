...
11. Barrida de código muerto (a pedido explícito, no parte de la limpieza incremental de `main.js`): borrados 5 archivos completos sin ninguna referencia en el repo — `state/appState.js`, `ui/welcomeView.js` (scaffold viejo de la migración a Vite), `ui/labValues.js` (607 líneas, superado por `ui/quickReferenceData.js`), `ui/examQuestionRenderer.js` (257 líneas, implementación de renderizado de preguntas nunca cableada), `config/supabase.js` (cliente Supabase alternativo vía npm, superado por `/supabase-global.js` que inicializa `window.sb` como script global). Se quitó la dependencia npm `@supabase/supabase-js` de `package.json` (había quedado huérfana, solo la usaba el archivo borrado) y se regeneró `package-lock.json`. Dentro de `main.js`: borradas `resiarVisibleQuestionType()` y `getQuestionImageUrl()` local (ninguna se llamaba nunca), más los imports huérfanos `getQuestionImagePaths`/`getQuestionImageLabel` que solo usaba esa función ya borrada. En `utils/questionImages.js`: borrado el export `getQuestionImageUrl` (nunca importado por nadie). **Nota, no se tocó:** hay duplicación de lógica real entre las versiones locales de `main.js` (`resiarGetQuestionImagesCacheVersion`, `resiarAppendQuestionImageCacheParam`, `getQuestionImageUrlFromPath`) y las exportadas/testeadas de `utils/questionImages.js` — ambos sets están en uso real por distintos call sites (main.js usa las suyas, `ui/explanation.js` usa las de utils/), así que no es código muerto, es candidato a refactor de unificación en otra sesión. Suite se mantuvo en 140 tests (sin tests nuevos, solo remoción). `main.js` 5.778→5.763 líneas. -1.031 líneas netas en el repo.

## Pendiente / próximo paso
- Seguir buscando bloques cohesivos en `main.js` (5.763 líneas todavía).
...
- Candidato de refactor (no código muerto): unificar las versiones
  locales de `main.js` para cache de imágenes de pregunta
  (`resiarGetQuestionImagesCacheVersion`, `resiarAppendQuestionImageCacheParam`,
  `getQuestionImageUrlFromPath`) con las exportadas/testeadas de
  `utils/questionImages.js`, que son casi idénticas pero con firma
  distinta (options param). Evaluar si vale la pena antes de tocarlo.
